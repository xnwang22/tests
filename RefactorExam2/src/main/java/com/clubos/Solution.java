...
class Solution {
    ...
    private static final int MAX_NUMBER_OF_QUESTIONS = 1000;
    private static final int MAX_NUMBER_OF_TOPICS = 1000;
    private static final int MAX_NUMBER_OF_QUERIES = 1000;
    ...
    private boolean validateInput(List<String> data) {
        ...
        if (numTopics > MAX_NUMBER_OF_TOPICS || numTopics < 1) {
            throw new IllegalArgumentException("the number of Topics is out of bounds");
        } else if (numQuestions > MAX_NUMBER_OF_QUESTIONS || numQuestions < 1) {
            throw new IllegalArgumentException("the number of Questions is out of bounds");
        } else if (numQueries > MAX_NUMBER_OF_QUERIES || numQueries < 1) {
            throw new IllegalArgumentException("the number of Queries is out of bounds");
        }
        ...
        returnValue = validateTopics(topics);
        returnValue = returnValue && validateQuestions(questions);
        returnValue = returnValue && validateQueries(queries);
